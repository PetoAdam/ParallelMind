...
void Network::backpropagate(const Matrix& input, const Matrix& target, float learningRate) {
    ...
    // For softmax+cross-entropy with linear logits, dL/dz = (softmax(z) - onehot).
    // We approximate by using error = (output - target), treating output as logits pre-softmax
    ...
    for (size_t epoch = 0; epoch < epochs; ++epoch) {
        ...
        for (size_t k = 0; k < idx.size(); ++k) {
            ...
            if (k % 100 == 0) {
                ...
                std::cout << "[Epoch " << (epoch+1) << ", Sample " << k << "] Pred=" << pred << ", True=" << gt << ", Loss=" << loss << std::endl;
            }
        }
        std::cout << "Epoch " << epoch + 1 << " complete. Acc: " << (inputs.empty()?0.0:(double)correct/inputs.size()) << std::endl;
    }
}
